...
class image_processing_node():
    ...
    def draw_lines(self,data):
        ...
        for k in xrange(1,numpoints+1,1):
            xIforward = (self.v_ref*dt*k/2)/0.3048+forward_offset
    ...
    def publish_states(self,data):
        ...
        self.compute_uOpt(leftlist, midpointlist, rightlist)
        ####################################################################
     ######################################################################################
    ...
    #            dt = self.avg2#self.dt
        ...
        #     print(ErrorMessage)
        #     #pass
    def compute_uOpt(self, leftlist, midlist, rightlist):
    	global l_d_last, r_d_last, Kp, last_cte, sum_cte
    	global weighted_cte

    	# Input to polynomial is y; output is x
    	left_poly = np.polyfit(leftlist[:,1],leftlist[:,0],4)
	mid_poly = np.polyfit(midlist[:,1],midlist[:,0],4)
    	right_poly = np.polyfit(rightlist[:,1],rightlist[:,0],4)

    	left_fit = np.polyval(left_poly, leftlist[:,1])
	mid_fit = np.polyval(mid_poly, midlist[:,1])
    	right_fit = np.polyval(right_poly, rightlist[:,1])

    	l_d = np.abs(left_fit - mid_fit / 2)
    	r_d = np.abs(right_fit - mid_fit / 2)

    	# CTE
    	cte = np.array(l_d - r_d)
	weighted_cte = []

    	# rospy.logwarn("Length of weighted_cte %d", len(weighted_cte))
    	# First 5 frames
    	if (len(weighted_cte) < 5):
            if cte.size == 20:
                weighted_cte.append(cte)
            return
    	cte = 0.6*weighted_cte[4] + 0.2*weighted_cte[3] + 0.1*weighted_cte[2] + 0.07*weighted_cte[1]
    	rospy.logwarn("error is \n : %s", np.array2string(cte))

    	Kp = np.array([1, 0.8, 0.2])
    	Ki = np.array([0, 0, 0])
    	Kd = np.array([3, 1, 1])

    	P_effect = np.sum(np.multiply(Kp, cte))

    	# Integral
    	sum_cte = sum_cte + cte
    	I_effect = np.sum(np.multiply(Ki, sum_cte))

    	# Derivative
    	D_effect = np.sum(np.multiply(Kd, cte - last_cte))
    	last_cte = cte

    	vOpt = 0.0
    	deltaOpt = P_effect + I_effect + D_effect

    	# Pop first weighted CTE and queue last
    	weighted_cte.popleft()
    	if cte.size == 20:
            weighted_cte.append(cte)
